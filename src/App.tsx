...
export default function App() {
  ...
  const [receiverBalance, setReceiverBalance] = useState<Number | undefined>(
    undefined
  );
  ...
  const createSender = async () => {
    ...
    var senderKeypair = Keypair.generate()
  ...
  const transferSol = async () => {    
    ...
    let balance = await connection.getBalance(receiverPublicKey!) / LAMPORTS_PER_SOL;
          ...
          {provider && receiverPublicKey && (
            <div>
              ...
              <div style={{
                  ...
                  top: "70px",
                  right: "28px"
                }}>Balance: {receiverBalance? receiverBalance: 0}</div>
            </div>
          )}
